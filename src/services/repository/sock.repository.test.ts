...
describe('Given TaskApi Service', () => {
    describe('When we instantiate it', () => {
        ...
        test(`Then if I use service.create()
            ...
            global.fetch = jest.fn().mockResolvedValue({
                ok: true,
                json: jest.fn().mockResolvedValue(mockSock),
            });
            const result = await service.createSock(mockSock);
            expect(fetch).toHaveBeenCalled();
            expect(result).toEqual(mockSock);
        });
        ...
        test('Then if I use service.deletesneaker() it should return an undefined', async () => {
            ...
            const result = await service.deleteSock(mockSock.id);
            expect(fetch).toHaveBeenCalled();
        ...
        test('Then if I use service.update.sneaker it should return', async () => {
            ...
            const result = await service.updateSock(mockSock);
